...
def generator(noise_features, features, channels=1):
    G = nn.Sequential(
        # Batch_size x noise_features x 1 x 1
        nn.ConvTranspose2d(noise_features, features * 4,
...
if __name__ == '__main__':
    ...
    class_1_label = torch.tensor(np.tile([0, 1], [batch_size,1]))
    class_0_label = torch.tensor(np.tile([1, 0], [batch_size,1]))
    label_size = class_1_label.shape[-1]
